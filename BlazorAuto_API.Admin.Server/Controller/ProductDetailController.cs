...
using static BlazorAuto_API.Admin.Abstract.IProductDetailService;
...
public class ProductDetailController : ControllerBase, IProductDetailService
{
    ...
    public async Task<ResultOf<ProductDetailModel>> FindByGuid(Guid Guid)
    {
        try
        {
            ...
            var rps = new ProductDetailModel()
            {
                ...
                CategoryGuids = [.. productDetail.ProductCategories.Select(spec => spec.Guid)],
    ...
    public async Task<ResultsOf<CategoriesInfoModel>> GetListCategory()
    {
        try
        {
            using ApplicationDbContext db = _DbContext.Connection();
    ...
    public async Task<Result> Update([FromBody] ProductUpdateModel Request)
    {
        try
        {
            ...
            else
            {
                ...
                foreach (var (curr, req) in updateValue)
                {
                    curr.IsEnable = req.IsEnable;
                    curr.Name = req.Name;
                    curr.Price = req.Price;
                    curr.DicountPrice = req.DicountPrice;
    ...
    public async Task<Result> UpdateImage([FromQuery] ReqInfoUpdateImage reqInfo, [FromForm] IList<IFormFile> UploadFile)
    {
        using var db = _DbContext.Connection();
        var product = await db.Set<EntityProduct>()
            .FirstOrDefaultAsync(x => x.Guid == reqInfo.Guid);
        if (UploadFile == null && UploadFile!.Count != 1)
        {
            return "Mỗi lần update chỉ 1 ảnh";
        }
        if (product == null)
        {
            return "Không tìm thấy sản phẩm";
        ...
        try
        {
            using var streamImage = UploadFile[0].OpenReadStream();
            var image = await Image.LoadAsync(streamImage);
            var fouderRelLink = Path.Combine("Images", reqInfo!.Guid!.ToString()!);
            var fouderLink = Path.Combine(WebRootPath, fouderRelLink);
    ...
    public async Task<Result> RemoveImage(ReqDeleteImages Request)
    {
        ...
        try
        {
            ...
            foreach (var image in imagesToRemove)
            {
                ...
                proc.Images.Remove(image);
                proc.CarouselImages.RemoveAll(imageTemp => imageTemp.Guid == image.Guid);
            }
            db.Update(proc);
            await db.SaveChangesAsync();
            return Result.Ok();
